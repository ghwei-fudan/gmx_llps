...
void assembly::initOptions(IOptionsContainer          *options,
    ...
    options->addOption(FileNameOption("pdb_system")
                               .filetype(eftPDB).outputFile()
                               .store(&fnAllMoleculesPDB_)
                               .description("The Largest Cluster of the Last Frame"));
...
*/
 ...
 == mapped_unprocessedAtoms.end()) {
    ...
    if (!fnAllMoleculesPDB_.empty() && fr.time == time_pdb_) {
        ...
        // We now open a pdb file
        FILE *fpPDB = fopen(fnAllMoleculesPDB_.c_str(), "w");
        gmx_write_pdb_box(fpPDB, pbc->ePBC, pbc->box);

        // We now process the largest cluster to find the centroid

        // We first determine the number of atoms in this cluster.
        int atomNumber = 0;
        for (int moleculeIndex : clusterList[0]) {
            atomNumber += idMap[moleculeIndex].size();
        }
        cout << "Number of all atoms in this cluster are: " << atomNumber << endl;
...
== mapped_unprocessedAtoms.end()) {
        ...
        double translate_vector_x = pbc->box[0][0] / 2 - centroid_x;
        double translate_vector_y = pbc->box[1][1] / 2 - centroid_y;
        double translate_vector_z = pbc->box[2][2] / 2 - centroid_z;

        int fragmentCount = 1;
        int atomCount = 1;

        double temp_atom_x, temp_atom_y, temp_atom_z;

        for (int atomIndex = 0; atomIndex < sel_.atomCount(); atomIndex++) {

            temp_atom_x = sel.coordinates()[atomIndex][0] + translate_vector_x;
            if (temp_atom_x > pbc->box[0][0]){
                temp_atom_x = temp_atom_x - pbc->box[0][0];
            }
            if (temp_atom_x < 0){
                temp_atom_x = temp_atom_x + pbc->box[0][0];
            }

            temp_atom_y = sel.coordinates()[atomIndex][1] + translate_vector_y;
            if (temp_atom_y > pbc->box[1][1]){
                temp_atom_y = temp_atom_y - pbc->box[1][1];
            }
            if (temp_atom_y < 0){
                temp_atom_y = temp_atom_y + pbc->box[1][1];
            }

            temp_atom_z = sel.coordinates()[atomIndex][2] + translate_vector_z;
            if (temp_atom_z > pbc->box[2][2]){
                temp_atom_z = temp_atom_z - pbc->box[2][2];
            }
            if (temp_atom_z < 0){
                temp_atom_z = temp_atom_z + pbc->box[2][2];
            }
